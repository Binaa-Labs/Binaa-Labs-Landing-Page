...
export default function ClientEffects() {
  useEffect(() => {
    // B5-low (TBT recovery, PROJECT.md D25): this setup — a document-wide
    // querySelectorAll, an IntersectionObserver over every [data-reveal]
    // element on the page, and the magnetic-button listeners — used to run
    // as part of the same passive-effects flush React schedules right after
    // the initial hydration commit, so it landed inside the same busy
    // window Lighthouse charges to TBT. Deferring it via
    // requestIdleCallback lets hydration's own commit finish first; the
    // bounded timeout (150ms, well under the hero's 600ms `--dur-slow`
    // entrance) keeps the very first reveal (desktop hero, which unlike
    // mobile still depends on this observer per D19/2b) imperceptibly
    // delayed rather than stalled indefinitely under sustained load.
    let cancelled = false;
    ...
    const idleHandle = hasRIC
      ? window.requestIdleCallback(setup, { timeout: 150 })
